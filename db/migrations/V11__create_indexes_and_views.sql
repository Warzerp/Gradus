...
CREATE INDEX IF NOT EXISTS idx_tg_embedding_hnsw
    ON trabajos_grado
    USING hnsw (embedding vector_cosine_ops)
    WITH (m = 16, ef_construction = 64);

-- ─── Índices de búsqueda textual frecuente ────────────────────────────────────
CREATE INDEX IF NOT EXISTS idx_tg_titulo      ON trabajos_grado USING gin(to_tsvector('spanish', titulo));
CREATE INDEX IF NOT EXISTS idx_autores_nombre ON autores (nombre, apellido);
CREATE INDEX IF NOT EXISTS idx_usuarios_email ON usuarios (email);
CREATE INDEX IF NOT EXISTS idx_auditoria_ts   ON auditoria (registrado_en DESC);

-- ─── Vista materializada de estadísticas por facultad ────────────────────────
-- Se pobla en Sprint 3; aquí solo se define la estructura
CREATE MATERIALIZED VIEW IF NOT EXISTS mv_stats_por_facultad AS
SELECT
